...
\begin{figure}[H] %abstrakt struktur/kommunikationssetup
	\centering
	\begin{tikzpicture}
		% timelines
		\draw	[dashed]	(0,5)	--	(0,0)
...
\begin{figure}
	...
	\begin{tikzpicture}[->,>=stealth', node distance=3.5 cm, thick]
		...
		\node[state, initial]	(A)	[rectangle,	rounded corners]					{No map loaded};
		\node[state]			(B)	[rectangle,	rounded corners,below left of=A]	{Awaiting Map};
		\node[state]			(C)	[rectangle,	rounded corners,below right of=A]	{Map loaded};

		\path	(A)	edge[bend right,left]									node{Request Map}	(B)
					edge[loop above,left,		out=130,in=100,	looseness=7]node{Trash Map}		(A)
					edge[loop right,right,		out=50,	in=80,	looseness=7]node{Await Response}(A)
				...
				(C)	edge[bend right,above]									node{Request Map}	(B)
					...
					edge[loop below,below right,out=300,in=270,	looseness=7]node{Await Response}(C);
	\end{tikzpicture}
...
\begin{figure}
	...
	\begin{tikzpicture}[->,>=stealth', node distance=3.5 cm, thick]
		...
		\node[state, initial]	(A)	[rectangle,	rounded corners]			{Idle};
		\node[state]			(B)	[rectangle,	rounded corners,right of=A]	{Active};

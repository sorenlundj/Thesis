...
\begin{figure}
	...
	\begin{tikzpicture}[->,>=stealth', node distance=3.5 cm, thick]
		...
		\path (A) edge [bend left,  above] node {Insert Coin} (B)
							edge [loop above, above] node {Push}        (A)
					(B) edge [bend left,  below] node {Push}        (A)
							edge [loop above, above] node {Insert Coin} (B);
	\end{tikzpicture}
	\caption{Example of a finite state machine, describing a turnstile. This figure relates to Table \ref{tab:fsmEx}.}
	\label{fig:fsmEx}
\end{figure}
...
\begin{table}[h!]
	...
	\end{tabular}
	\caption{Example of a finite state machine, describing a turnstile. This table relates to Figure \ref{fig:fsmEx}.}
	\label{tab:fsmEx}
\end{table}

...
\noindent
A visual representation of the points, described are displayed in Figure \ref{fig:protocolVisual}, where Sub-figure \ref{subfig:manual} represents the manually created model, and \ref{subfig:automatic} represents the automatically created model.

\begin{figure}[h]
  \begin{subfigure}{.48\textwidth}
    \begin{tikzpicture}[->,>=stealth', node distance=2.5 cm, thick]
      \tikzstyle{every state}=[draw=black,rectangle,rounded corners]
  
      \node[state] (A) [align=center]            {ship\\   $<0.78.0>$\\$["map"]$};
      \node[state] (B) [align=center,below of=A] {service\\$<0.79.0>$};
      \node[state] (C) [align=center,below of=B] {company\\$<0.80.0>$\\$["map"]$};
  
      \path (A) edge[bend right,left] node[align=left]{fun aux:user/1\\
                                                       fun aux:psswd/1} (B)
            (B) edge[bend right,left] node{} (A)
                edge[bend right,left] node{} (C) 
            (C) edge[bend right,left] node{} (B);
    \end{tikzpicture}
    \caption{}
    \label{subfig:manual}
  \end{subfigure}
  \begin{subfigure}{.48\textwidth}
    \begin{tikzpicture}[->,>=stealth', node distance=2.5 cm, thick]
      ...
      \node[state] (A) [align=center]            {ship\\   $<0.85.0>$\\$["map"]$};
      \node[state] (B) [align=center,below of=A] {service\\$<0.84.0>$};
      \node[state] (C) [align=center,below of=B] {company\\$<0.83.0>$\\$["map"]$};
    ...
    \end{tikzpicture}
    \caption{}
    \label{subfig:automatic}
  \end{subfigure}
  \caption{Visual representation of Figures \ref{fig:protocolManual}, and \ref{fig:protocolAutomatic}.}
  \label{fig:protocolVisual}
\end{figure}
%TODO: write about equiv_fsms here
Having concluded that the resulting models\footnote{Models are collections of all states in the FSM} are equal, in order for the two FSMs to be equivalent, all available actions must also be equal. The functionality of the automatic implementation is build to utilize the manual model building module, and thus, their functionalities are near identical, only limited by the functionality of the parser and interpreter's ability to feed instructions to \ttt{mmods.erl}.
%{automatic bruger manual, så funktionaliteten bagved er OGSÅ ens - ergo er samlingerne af states (modellerne) og samlingerne af events (funktionaliteten) ens og så \tit{maa} de to fsms være ens}
